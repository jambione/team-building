...
_Required when `Affected Repos` includes more than one repo._

| Event          | Repo           | Branch                   | Commit SHA | Timestamp (UTC) | Owner | Status        |
| -------------- | -------------- | ------------------------ | ---------- | --------------- | ----- | ------------- |
| Branch created | `<owner/repo>` | `mission/<mission-slug>` | —          | HH:MM           | riker | done / failed |
| Wave complete  | `<owner/repo>` | `mission/<mission-slug>` | `<sha>`    | HH:MM           | riker | done / failed |
...
- [ ] Multi-repo missions include a completed Multi-Repo Execution Log with per-repo commit SHA evidence
- [ ] Multi-repo missions filed `knowledge_base/missions/manifests/YYYY-MM-DD-<mission-slug>.manifest.md`
